...
- **Security regression fix:** reverted `encryption_at_host_enabled` default from `false` back to `true`. The `false` default (introduced in Nov 2024) silently disabled host encryption for any caller not explicitly setting this field. Callers relying on the `false` default must now set `vmss.encryption_at_host_enabled = false` explicitly.
  - **Upgrade action required:** this produces an in-place update (not a destroy/recreate) on `terraform plan` for every existing deployment that doesn't already set `encryption_at_host_enabled` explicitly. Before applying, confirm the target VM size and region [support Encryption at Host](https://learn.microsoft.com/azure/virtual-machines/disks-enable-host-based-encryption-portal#supported-vm-sizes) — sizes/regions without support will fail the apply, not just the plan. If unsupported, set `vmss.encryption_at_host_enabled = false` in tfvars before upgrading.
